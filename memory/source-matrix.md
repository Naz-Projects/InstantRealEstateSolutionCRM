...
| 2 | **Code violations / property-maintenance** | `CustomMaps/CodeEnforcement_CodeCases/MapServer/0` ("CODE CASES"); helper views in `CustomMaps/Code_Enforcement/MapServer` (9 Open Cases, 11 Cases-added-last-30-days) | **Free API** | **YES — verified.** Cursor = **`APDTTM`** via `where=APDTTM > TIMESTAMP 'YYYY-MM-DD HH:MM:SS'` (returned 748 since 2026-05-01 — sane subset of 2,852). ⚠ `last_edited_date` is sparsely populated (same query → 0) and **epoch-ms `where` 400s** — use the `TIMESTAMP '…'` literal. | `PRCLID`, `ADDR`, `APTYPE`/`APDESC` (e.g. "HIGH WEED AND GRASS"), `STAT` (O=open), `APDTTM`, `created_date`, `last_edited_date`, `YEARSOPEN`, `INSPECTIONS` | ✅ 2,852 cases (1,051 added in last 30d) | Free, public |
...
| 5 | **Sheriff sales (structured — SUPPLEMENTS the PDF, ≠ replacement)** | `CustomMaps/SheriffSales/MapServer/0` ("Sheriff Sales") | **Free API** | Re-pull monthly (small) | `PARCELID`/`PRCLID`, `ADDRESS`, `MAILING_LOCATION`, `CASENUMBER`, `PLANTIFF`, `ATTORNEY`, `COMMENTS`. ⚠ **NO sale type (TAX/MTG/JUDG), NO principal/judgment amount, NO sale date** — which `deal.ts` cushion math needs — so it CANNOT replace the PDF parse; it adds a clean `PARCELID` join + court `CASENUMBER`/`PLANTIFF` the PDF lacks. | ✅ 53 (matches live PDF) | Free, public |
...
| 8 | **Assessed / market value** | NCC **parcel lookup / "personal search"** tool `www3.newcastlede.gov/parcel/search/` (Reblaze) — **NOT on any free ArcGIS layer** (confirmed: NCC `BaseMaps/0` and FirstMap `DE_StateParcels` both lack it). Live aggregators `newcastlecounty.io` / `mydetax.com` scrape this same tool per-address (evidence it's public but per-parcel). | **Browser** (Firecrawl browser-actions / Scrapling) or **paid** (ATTOM/CoreLogic) | Per-parcel only | assessed value, sales history, year built, beds/baths, building sketch, **tax + sewer balances** | ✅ per-parcel | Free-but-browser (funnel-only) or paid |
...
- **★ Tax / sewer balances + assessed value — is there a FREE BULK source we haven't checked? (user's hunch — NOT yet probed).**
  KNOWN: it's public, but only via the **per-parcel "personal search" tool** (rows 8–9; Reblaze → browser, one house at a time);
  "public" does **not** equal "free bulk feed." NOT yet ruled out (probe these BEFORE defaulting to per-parcel browser/paid):
  (a) the **token-secured `PublicWorks` (likely sewer) + `LU_Eng` (likely assessment/land-use) ArcGIS folders** — 499 Token
  Required; is a public/obtainable token available? (b) the **Sale Lists / monition PDFs** (row 12) already carry *delinquent*
  tax balances for the distressed subset — free, and that subset is exactly who we want; (c) a county **bulk export / open-data
  CSV** (the GIS Downloads page + the user's mentioned CSV export — may include an assessment/tax table); (d) a state/FirstMap
  assessment dataset. ⇒ first probe-list when we tackle the value/balance layer (Phase 5, or sooner if it unlocks cheap equity scoring).
...
3. **Quick win available (separate, optional):** **augment** (not replace) the existing Sheriff-PDF parse with the
   structured `SheriffSales/0` layer — join its clean `PARCELID` + court `CASENUMBER`/`PLANTIFF` onto the scraped rows.
   The layer **lacks** sale type / principal / sale date (which `deal.ts` needs), so the PDF parse stays the source of truth.

...
## ★★★★★★★ START HERE — 2026-07-01 — "MONITOR THE WEB" (Zillow NCC deal finder) — **SHIPPED TO PROD + PEN-TESTED LIVE (working). NEXT: (a) keeper-precision tuning, (c) register the Firecrawl Monitor webhook.**
...
**STATUS: LIVE ON PROD + PEN-TESTED (works).** Merged `feat/monitor-web-zillow` → `origin/main` **`72eed27`** (fast-forward from 36b1dcc). Prod backend deployed to `pastel-crocodile-994` (2 tables + indexes, functions, `/firecrawl-monitor` http route, daily cron); frontend live via the Cloudflare build. `FIRECRAWL_WEBHOOK_SECRET` set on prod (value saved gitignored at `.superpowers/sdd/firecrawl_webhook_secret.txt` in the worktree). `FIRECRAWL_API_KEY`+`OPENROUTER_API_KEY` already on prod. **312 tests, build clean.** ⚠ **This is DONE — do NOT rebuild.** If your repo checkout is on the old `main` (36b1dcc), `git checkout main && git pull` first (origin/main = 72eed27; the worktree `.claude/worktrees/monitor-web` holds the feature branch).

**Final review + fixes (commit `72eed27`, on top of the subagent build):** the final whole-branch review found 1 Critical + 3 Important — all FIXED: **#1** dropped `|| verdict?.keep` from the keeper `distress` term (the LLM no longer overrides the deterministic gate — design: "DeepSeek judges condition/distress, NEVER the math"); **#2** house-number guard in `crossRefOffMarket` (leading-digit match vs `parcel.situsStreet` → no false off-market badge on a same-street mismatch) — this was the OLD #1 fast-follow, now DONE; **#3** missing `FIRECRAWL_API_KEY` fails safe (recorded failed run + `logServerError`; webhook acks 200, not 500).

**LIVE PROD PEN-TEST (2026-07-01, ~150 Firecrawl credits):** `runMonitorScan {trigger:"manual",maxPages:1}` → **41 scanned → 24 new → 24 analyzed / 0 failed.** Insights correct & investor-grade: 16 keepers ranked by dealScore; top = 16 E 24th / 309 Cedar / 218 W 23rd (score 72, RENTAL, cap 6.3–6.8%, flip correctly -ve, comps-capped ARV, agent contacts, price history, DeepSeek reason). **Fix #1 confirmed live** (218 W 23rd: `aiKeep=false` but `keeper=true` via the rental cap — math decides, not the LLM). `/monitor` UI polished (dark theme, lucide-only, "Keepers (16) / All new (24)" tabs, Details/Promote/Flip/Zillow). **Promote-to-Potential works** (test-promoted **309 Cedar St** → sits in `/potential` "To work", source Manual, value=ARV, chips carried — a real record; keep it or move to Dead). **Security:** requireUser gates reject the CLI (UNAUTHENTICATED); prod webhook fails closed (no-sig / bad-sig → 401, GET / nonexistent → 404). Full write-up in the branch ledger `.superpowers/sdd/progress.md`.

### ★ NEXT ACTIONS — user said "do all three next session" (memory=done; these two remain)
1. **(a) KEEPER-PRECISION TUNING → redeploy.** ~9/16 keepers score 0–20 `bestExit:PASS`, kept ONLY because DeepSeek tagged AS-IS/estate boilerplate as "distressed"; a couple (508 Lake Dr, 513 W 37th) are listed ABOVE market (negative spread) yet kept. Honest (they sort last, labeled PASS/score 0) but noisy. Tighten the `distress`→keep path in `decideKeeper` (`src/scraper/monitorListings.ts`): keep a distress-only listing ONLY when spread ≥ 0 (not above market) OR dealScore ≥ a floor (~30). Pure-logic + tests, then redeploy prod (`convex deploy` w/ prod key + push `main`) + re-scan to confirm the tail shrinks. Build via subagent (**model = user's choice — this session used Sonnet 5 per a user directive; standing default is Opus [[implementation-via-opus-subagents]]** — confirm which).
2. **(c) REGISTER THE FIRECRAWL MONITOR (real-time webhook auto-scan).** `CONVEX_DEPLOY_KEY=<prod> npx convex run monitorActions:createFirecrawlMonitor '{}'` → registers a Firecrawl Monitor on the NCC search that POSTs to prod `https://pastel-crocodile-994.convex.site/firecrawl-monitor` with the HMAC secret. **Verify Task-15 `createFirecrawlMonitor`'s args + endpoint against the current Firecrawl `/v2/monitor` docs before running.** The daily cron `0 2 * * *` is the safety net today; the webhook makes it fire on new listings.
3. **(b) MEMORY — DONE this session** (this file + `memory/{memory,todo,lessons}.md` + auto-memory `[[monitor-web-zillow]]`).
...
- Minor visual: one card (218 W 23rd) rendered a broken-image placeholder (missing/404 photo URL).
- Cosmetic/low-risk (ledger): `monitorRuns` keeperCount/analyzedCount stay 0 (set at finishRun before the async fan-out); `upsertListing` no-advance on price INCREASE (stale-favorable math); comps cache zip+TTL; dead `wholesaleSpread`/WHOLESALE chip + `propCity`; mislabeled Task-4 test name. External `firecrawl` skill already refreshed (v2/proxy/webhook/Monitor).

### ⚠ CONCURRENCY EPISODE THIS SESSION (see lessons.md 2026-07-01) — how the build actually happened
A **persistent forked "twin" controller** (spawned by a mid-run process restart, session id `40674cfd`→`40af4b9d`) ran the SAME subagent-driven plan in the SAME worktree, autonomously and CORRECTLY, building Tasks 2–15 + its own reviews/fixes/memory while the live session watched. Racing it corrupts the branch; standing hands-off (single commit lineage) + monitoring to completion was the safe play. Background watcher bash jobs kept getting **killed by process-lifecycle events** → a self-healing relaunch loop tracked progress. The live session then took over for the final review + prod deploy + pen-test.
